  ...
  module Regex
    ...
    def self.email
      @email_regex ||= begin
        ...
        /\A#{full_email}(\s*,\s*#{full_email})*\z/i
      end
    end

    def self.http_url
      @http_url ||= Regexp.new("\\A(?:http|https):(?:#{URI::DEFAULT_PARSER.pattern[:NET_PATH]})(?:\\?(?:#{URI::DEFAULT_PARSER.pattern[:QUERY]}))?\\z", Regexp::EXTENDED, 'N')
    end
  end
